...
router.get('/executive-summary', async (_req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const [
      ...
      db.select({ total: sum(bookings.totalAmount) }).from(bookings).where(and(gte(bookings.createdAt, lastMonthStart.toISOString()), lte(bookings.createdAt, lastMonthEnd.toISOString()))),
      db.select({ total: sum(bookings.totalAmount) }).from(bookings).where(gte(bookings.createdAt, thisMonthStart.toISOString())),
      db.select({ total: sum(bookings.totalAmount) }).from(bookings).where(and(gte(bookings.createdAt, lastYearStart.toISOString()), lte(bookings.createdAt, lastYearEnd.toISOString()))),
      db.select({ total: sum(bookings.totalAmount) }).from(bookings).where(gte(bookings.createdAt, thisYearStart.toISOString())),
